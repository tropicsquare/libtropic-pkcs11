...
Two SPI HAL variants are available:
 ...
 - `linux_spi_native_cs` uses the SPI driver's native chip select instead of controlling it separately via GPIO.
...
This configuration utilizes native chip select handling of the SPI driver. GPIO is optionally used for interrupt only.
