...
CK_RV C_Initialize(CK_VOID_PTR pInitArgs)
{
    ...
    int dev_path_len = snprintf(pkcs11_ctx.lt_device.dev_path, sizeof(pkcs11_ctx.lt_device.dev_path), "%s", TS_USB_DEV);
    if (dev_path_len < 0 || (size_t)dev_path_len >= sizeof(pkcs11_ctx.lt_device.dev_path)) {
        fprintf(stderr,
                "Error: TS_USB_DEV is too long for dev_path buffer (limit is %zu bytes).\n",
                sizeof(pkcs11_ctx.lt_device.dev_path));
        return -1;
    }
